document.addEventListener("DOMContentLoaded", function () {
  ...
  const settlementPOI = [
    ...
    },
    {
      ...
      link: "https://example.com/",
      description: "A shop specializing in potions, elixirs, and alchemical ingredients, frequented by adventurers and scholars alike."
    },
    {
      name: "Celestial Observatory",
      link: "https://example.com/",
      description: "A facility equipped with powerful telescopes for observing celestial phenomena and studying the movements of the stars."
    },
    {
      name: "Grand Bazaar",
      link: "https://example.com/",
      description: "A bustling marketplace where merchants from across the land gather to trade goods, spices, and exotic wares."
    },
    {
      name: "Arcane Nexus",
      link: "https://example.com/",
      description: "A focal point of magical energy where ley lines converge, attracting wizards, sorcerers, and seekers of arcane knowledge."
    },
    {
      name: "Artisan's Quarter",
      link: "https://example.com/",
      description: "A district known for its skilled craftsmen and artisans, producing fine goods ranging from jewelry to weapons and armor."
    },
    {
      name: "Library of Lore",
      link: "https://example.com/library_of_lore",
      description: "A vast repository of books, scrolls, and tomes containing knowledge on history, lore, and various academic disciplines."
    },
    {
      name: "Natural Springs",
      link: "https://example.com/natural_springs",
      description: "A serene oasis with crystal-clear waters believed to possess healing properties, drawing travelers seeking rest and rejuvenation."
    },
    {
      name: "Adventurer's Guildhall",
      link: "https://example.com/adventurers_guildhall",
      description: "A guildhall serving as a hub for adventurers to find quests, companions, and resources for their journeys."
    },
    {
      name: "Amphitheater",
      link: "https://example.com/amphitheater",
      description: "An open-air theater where performances, debates, and public gatherings take place, enriching the cultural life of the settlement."
    },
    {
      name: "Guardian Statue",
      link: "https://example.com/guardian_statue",
      description: "A towering statue dedicated to a legendary hero or mythical creature, symbolizing protection and strength for the community."
    },
    {
      name: "Astrologer's Tower",
      link: "https://example.com/astrologers_tower",
      description: "A tower inhabited by astrologers and fortune-tellers who offer insights into fate and destiny through the study of celestial bodies."
    },
    {
      name: "Grove of Tranquility",
      link: "https://example.com/grove_of_tranquility",
      description: "A peaceful grove surrounded by ancient trees, frequented by meditators, druids, and those seeking solace in nature."
    },
    {
      name: "Merchant Caravansary",
      link: "https://example.com/merchant_caravansary",
      description: "A fortified inn and trading post where merchant caravans stop to rest and conduct business during their travels."
    },
    {
      name: "Falconer's Roost",
      link: "https://example.com/falconers_roost",
      description: "A training ground for falconers and their birds of prey, offering demonstrations and hunts for visitors."
    },
    {
      name: "Hall of Champions",
      link: "https://example.com/hall_of_champions",
      description: "A hall adorned with trophies and memorabilia honoring legendary warriors, athletes, and adventurers from throughout history."
    },
    {
      name: "Wharf District",
      link: "https://example.com/wharf_district",
      description: "A waterfront area bustling with activity, where fishermen, sailors, and traders ply their trade and ships come and go."
    },
    {
      name: "Dreamer's Retreat",
      link: "https://example.com/dreamers_retreat",
      description: "A retreat center where travelers can explore their dreams and delve into the realm of the subconscious under the guidance of dream interpreters."
    },
    {
      name: "Clock Tower",
      link: "https://example.com/clock_tower",
      description: "A towering structure housing a clock mechanism that chimes the hours, serving as a landmark and timekeeping device for the settlement."
    },
    {
      name: "Enchanter's Workshop",
      link: "https://example.com/enchanters_workshop",
      description: "A workshop where skilled enchanters imbue objects with magical properties, catering to the needs of wizards, nobles, and adventurers."
    },
    {
      name: "Martial Arts Dojo",
      link: "https://example.com/martial_arts_dojo",
      description: "A training facility where martial artists hone their skills in various combat techniques, offering classes and tournaments for enthusiasts."
    }
  ];
  ...
  //   End Settlement Arrays
    ...
    enableSettlementsCheckbox.addEventListener("change", function () {
        toggleInputs(enableSettlementsCheckbox.checked, settlementsSelect, settlementPOISelect, racesSelect, encountersSelect);
    });

    function toggleInputs(checked, ...inputs) {
    ...
    worldForm.addEventListener("submit", function (event) {
        ...
        const settlementsCount = getCount(settlementsSelect);
        const settlementPOICount = getCount(settlementPOISelect);
        const racesCount = getCount(racesSelect);
        const encountersCount = getCount(encountersSelect);

        const selectedSettlements = getRandomElements(settlements, settlementsCount);

        let content = "";

        selectedSettlements.forEach((selectedSettlement, index) => {
            content += `
                <div class="row">
                    <div class="col-md-6">
                        <h4 class="text-center"><strong>${selectedSettlement.name}</strong></h4>
                        <p class="text-center">${selectedSettlement.description}</p>
                    </div>
                    <div class="col-md-6">
                        <div class="container">
                            <h5><strong>Points of Interest:</strong></h5>
                            <ul>
                                ${getRandomElements(selectedSettlement.poi, settlementPOICount).map(poi => `
                                    <li><strong>${poi.name}:</strong> ${poi.description}</li>`).join("")}
                            </ul>
                        </div>
                        <div class="container">
                            <h5><strong>Inhabitants:</strong></h5>
                            <ul>
                                ${getRandomElements(selectedSettlement.races, racesCount).map(race => `
                                    <li>${race}</li>`).join("")}
                            </ul>
                        </div>
                        <div class="container">
                            ...
                            <ul>
                                ${getRandomElements(encounters, encountersCount).map(encounter => `
                                    <li><strong>${encounter.name}:</strong> ${encounter.description}</li>`).join("")}
                            </ul>
                        </div>
                    </div>
                </div>`;

            if (index < settlementsCount - 1) {
                content += `<hr>`;
            }
        });
    ...
    function getCount(select) {
        return select.value === "random" ? Math.floor(Math.random() * 10) + 1 : parseInt(select.value);
    }
