...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateClearButtonVisibility() {
        clearButton.style.display =
            diceContainer.children.length > 0 ? "block" : "none";
    }

    // Clear button click event
    clearButton.addEventListener("click", function () {
        diceContainer.innerHTML = ""; // Clear the displayed results
        updateClearButtonVisibility(); // Update the Clear button visibility
    });
    ...
    form.addEventListener("submit", function (event) {
        ...
        // Display individual rolls one by one with a rolling effect
        let currentDelay = 500;
        rolls.forEach((roll, index) => {
            setTimeout(() => {
                // Create a new box element for the rolled number
                const rollBox = document.createElement("div");
                rollBox.classList.add("dice-box");

                // Set the content of the box to the current rolled number
                rollBox.textContent = roll;

                // Append the box to the dice container
                diceContainer.appendChild(rollBox);
        ...
        // After rolling animation completes, display the final rolled number
        setTimeout(() => {
            // Remove all previously displayed boxes
            diceContainer.innerHTML = "";

            // Create a new box element for the final rolled number
            const finalRollBox = document.createElement("div");
            finalRollBox.classList.add("dice-box");
            finalRollBox.classList.add("final-roll");

            // Set the content of the box to the final rolled number
            finalRollBox.textContent = totalRoll;

            // Append the final box to the dice container
            diceContainer.appendChild(finalRollBox);

            if (shouldPlaySound()) {
                diceSound.play(); // Play sound effect for the final roll
            }
