document.addEventListener("DOMContentLoaded", function () {
  ...
  content += selectedRacesForSettlement
    ...
    const randomEncountersCount = Math.floor(Math.random() * 5) + 1;
    const selectedEncounters = getRandomElements(encounters, randomEncountersCount);
    content += selectedEncounters
        .map((encounter) => `
        <li>
          Encounter ${encounter}: Description goes here. <a href="#">(Learn more)</a>
        </li>`)
        .join("");
