document.addEventListener("DOMContentLoaded", function () {
  ...
  function getRandomElements(array, count) {
    const shuffledArray = array.map((item, index) => index).sort(() => 0.5 - Math.random());
  ...
  worldForm.addEventListener("submit", function (event) {
    ...
    if (settlementsCount > 0) {
      ...
      for (let i = 0; i < settlementsCount; i++) {
        ...
        const randomEncounters = getRandomElements(encounters, Math.floor(Math.random() * 5) + 1).map(encounterIndex => encounters[encounterIndex - 1]);
...
for (let j = 0; j < randomEncounters.length; j++) {
  const selectedEncounter = randomEncounters[j];

  content += `
    <li>
      <strong>${selectedEncounter.name}:</strong> ${selectedEncounter.description}
    </li>`;
}
