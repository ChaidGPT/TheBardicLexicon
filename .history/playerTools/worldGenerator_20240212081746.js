document.addEventListener("DOMContentLoaded", function () {
  ...
  worldForm.addEventListener("submit", function (event) {
    ...
    if (encountersCountInput.value === "random") {
      ...
      encountersCountCount = parseInt(encountersCountInput.value);
    }
  // ----------
    ...
    if (settlementsCount > 0) {
      ...
      for (let i = 0; i < settlementsCount; i++) {
        ...
        if (encountersCountCount === "random") {
          const randomEncountersCount = Math.floor((Math.random() * 4) + 1)
          selectedEncountersForSettlement = getRandomElements(encounters, randomRacesCount);
        } else {
          selectedEncountersForSettlement = getRandomElements(encounters, racesCount);
        }
