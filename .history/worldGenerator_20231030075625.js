document.addEventListener('DOMContentLoaded', function () {
    ...
    worldForm.addEventListener('submit', function (event) {
        ...
        generatedContent.innerHTML = `
            ...
            <ul>
                ${selectedSettlements.map(item => `<li>Settlement: ${item}</li>`).join('')}
                ${selectedAncients.map(item => `<li>Ancient: ${item}</li>`).join('')}
                ${selectedFeatures.map(item => `<li>Physical Feature: ${item}</li>`).join('')}
                ${selectedInhabitants.map(item => `<li>Inhabitant: ${item}</li>`).join('')}
            </ul>
        `;
    });
});